...
import { resolverFinalPessoal } from '../finais.js'
...
for (const cenario of cenarios) {
  ...
  test(`[${cenario.slug}] se tiver paraInvestigar, são 2+ perguntas não vazias`, () => {
    const lista = cenario.desfecho?.paraInvestigar
    if (!lista) return // opcional — nem todo cenário precisa ter
    assert.ok(lista.length >= 2, `${cenario.slug} paraInvestigar com menos de 2 perguntas`)
    for (const pergunta of lista) {
      assert.ok(typeof pergunta === 'string' && pergunta.length > 10, `${cenario.slug} paraInvestigar com pergunta vazia/curta demais`)
    }
  })

  test(`[${cenario.slug}] finais pessoais: todo papel com 'finais' tem 'resolucaoFinais' com casos válidos`, () => {
    for (const papel of cenario.papeis) {
      if (!papel.finais) continue
      assert.ok(papel.resolucaoFinais?.casos?.length > 0, `[${cenario.slug}] '${papel.slug}' tem finais mas não tem resolucaoFinais.casos`)
      const slugsValidos = new Set(papel.finais.map((f) => f.slug))
      for (const caso of papel.resolucaoFinais.casos) {
        const temFinal = !!caso.final
        const temPesos = !!caso.pesos
        assert.ok(temFinal !== temPesos, `[${cenario.slug}] '${papel.slug}' tem um caso de resolucaoFinais com 'final' e 'pesos' juntos, ou nenhum dos dois`)
        if (temFinal) {
          assert.ok(slugsValidos.has(caso.final), `[${cenario.slug}] '${papel.slug}' resolucaoFinais aponta pro final inexistente: '${caso.final}'`)
        } else {
          for (const slug of Object.keys(caso.pesos)) {
            assert.ok(slugsValidos.has(slug), `[${cenario.slug}] '${papel.slug}' resolucaoFinais.pesos aponta pro final inexistente: '${slug}'`)
          }
        }
      }
    }
  ...
  test(`[${cenario.slug}] finais pessoais: resolverFinalPessoal nunca falha nem devolve null pra combinações plausíveis de flags`, () => {
    for (const papel of cenario.papeis) {
      if (!papel.finais) continue
      const exposicoesDeTeste = [-5, -2, -1, 0, 1, 2, 5]
      const eventosDeclarados = papel.resolucaoFinais.casos.map((c) => c.quando?.desfechoEvento).filter(Boolean)
      const eventosDeTeste = [undefined, ...new Set(eventosDeclarados)]
      for (const exposicao of exposicoesDeTeste) {
        for (const desfechoEvento of eventosDeTeste) {
          const flags = desfechoEvento ? { exposicao, desfechoEvento } : { exposicao }
          const resultado = resolverFinalPessoal(papel, flags, 12345)
          assert.ok(resultado, `[${cenario.slug}] '${papel.slug}' resolverFinalPessoal devolveu null para flags=${JSON.stringify(flags)}`)
        }
      }
    }
  })
